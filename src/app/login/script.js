...
document.addEventListener('click', function(event) {
    // Verifica si el clic fue fuera del contenedor principal
    if (!mainContainer.contains(event.target)) {
        // Aplicar la clase para deslizar fuera
        mainContainer.classList.add('slide-out');

        // Opcional: Ocultar completamente después de la animación
        setTimeout(() => {
            mainContainer.style.display = 'none'; // Ocultar el contenedor después de la animación
        }, 500); // Debe coincidir con el tiempo de la transición
    }
});

// Ejecución de funciones existentes
